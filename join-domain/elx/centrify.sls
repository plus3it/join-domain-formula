...
{%- set centrifyPkg = pillar['ad_join']['package_name'] %}
{%- set centrifyHash = pillar['ad_join']['package_hash'] %}

# Vars for checking for previous installations
{%- set centrifyBinDir = pillar['ad_join']['install_bin_dir'] %}
{%- set centrifyVarDir = pillar['ad_join']['install_var_dir'] %}
{%- set centrifyDbDir = pillar['ad_join']['install_db_dir'] %}
{%- set centrifyDbs = pillar['ad_join']['checkFiles'] %}

{%- set centrifyRpms = pillar['ad_join']['connectorRpms'] %}
